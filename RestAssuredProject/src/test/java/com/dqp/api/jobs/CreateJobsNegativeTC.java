...
public class CreateJobsNegativeTC {
	...
	void invalidMeasureName() { //change measure name  //did not able to create
			
			RestAssured.given().contentType("application/json")
				.body("{\"job.name\":\"TEST_E2E_Completeness\",\"job.type\":\"batch\",\"measure.id\":211496,\"dq.type\""
/*change*/				+ ":\"COMPLETENESS\",\"measure.name\":\"TEST_E2E_Comp$$$eness\",\"projectId\":3,\"cron.expression\""
/*change*/				+ ":\"0 */5 * ? * *\",\"cron.time.zone\":\"GMT+5:30\",\"owner\":\"data_steward\",\"data.segments\""
/*change*/				+ ":[{\"data.connector.name\":\"source_1610426653360\",\"as.baseline\":true,\"segment.range\""
						+ ":{\"begin\":\"-500day\",\"length\":\"10day\"}}],\"runCount\":0}")
					.post(RestAssured.baseURI + "/api/v1/jobs").then().log().all();
				
		}
	
	@Test(priority = 1, enabled = false)   //Default project workspace   //false   true
	void invalidCronExpression() { //change measure name
			
			RestAssured.given().contentType("application/json")
				.body("{\"job.name\":\"TEST_E2E_Completeness\",\"job.type\":\"batch\",\"measure.id\":211496,\"dq.type\""
/*change*/				+ ":\"COMPLETENESS\",\"measure.name\":\"TEST_E2E_Completeness\",\"projectId\":3,\"cron.expression\""
/*change*/				+ ":\"#)#(*/5 * ? * *\",\"cron.time.zone\":\"GMT+5:30\",\"owner\":\"data_steward\",\"data.segments\""
/*change*/				+ ":[{\"data.connector.name\":\"source_1610426653360\",\"as.baseline\":true,\"segment.range\""
						+ ":{\"begin\":\"-500day\",\"length\":\"10day\"}}],\"runCount\":0}")
					.post(RestAssured.baseURI + "/api/v1/jobs").then().log().all();
				
		}
	@Test(priority = 2, enabled = false)   //Default project workspace
	void invalidTableName() { //was able to create but not seen on UI
...
/*change*/				+ "\\\"table\\\":\\\"lte_msisdn_leakage_daily_part_\\\",\\\"process\\\":\\\"BATCH\\\","
